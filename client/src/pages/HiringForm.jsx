...
import { FaUser, FaMapMarkerAlt, FaCalendarAlt, FaClock, FaFileAlt, FaPhone } from 'react-icons/fa';
  ...
  useEffect(() => {
    ...
    const fetchWorkerDetails = async () => {
      ...
      try {
        ...
        const response = await axios.get(`/api/workers/details/${workerId}`, config);
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-xl shadow-md overflow-hidden p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Hiring Request Sent!</h2>
          <p className="text-gray-600 mb-6">
            Your request has been sent to {worker?.name}. You will be notified when they respond to your request.
          </p>
          <button
            onClick={() => navigate('/')}
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-md overflow-hidden p-8">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-gray-800">Hire a Professional</h1>
          {worker && (
            <p className="mt-2 text-gray-600">
              You are about to hire <span className="font-semibold">{worker.name}</span>
            </p>
          )}
        </div>
